...
```{r}
writeLines(readLines(root("demos_rstan","bern.stan")))
```
...
```{r}
writeLines(readLines(root("demos_rstan","binom.stan")))
```
...
```{r}
writeLines(readLines(root("demos_rstan","binomb.stan")))
```
...
```{r}
writeLines(readLines(root("demos_rstan","binom2.stan")))
```
...
```{r}
d_kilpis <- read.delim(root("demos_rstan","kilpisjarvi-summer-temp.csv"), sep = ";")
...
```{r}
writeLines(readLines(root("demos_rstan","lin.stan")))
```
...
```{r}
writeLines(readLines(root("demos_rstan","lin_std.stan")))
```
...
```{r}
writeLines(readLines(root("demos_rstan","lin_t.stan")))
```

```{r, results='hide'}
fit_lin_std_t <- stan(file = 'lin_std_t.stan', data = d_lin, seed = SEED)
```
...
Compute the probability that the summer temperature is increasing.
```{r}
draws_lin_t <- extract(fit_lin_std_t, permuted = T)
mean(draws_lin_t$beta>0) # probability that beta > 0
```
...
```{r}
mu <- apply(draws_lin_t$mu, 2, quantile, c(0.05, 0.5, 0.95)) %>%
  t() %>% data.frame(x = d_lin$x, .)  %>% gather(pct, y, -x)
...
pfit <- ggplot() +
  ...
  labs(y = 'Summer temp. @Kilpisjärvi', x= "Year") +
  guides(linetype = F)
pars <- intersect(names(draws_lin_t), c('beta','sigma','nu','ypred'))
...
```{r}
writeLines(readLines(root("demos_rstan","grp_aov.stan")))
```
...
```{r}
writeLines(readLines(root("demos_rstan","grp_prior_mean.stan")))
```
...
```{r, results='hide'}
fit_grp <- stan(file = 'grp_prior_mean.stan', data = d_grp, seed = SEED, control=list(adapt_delta=0.95));
```
...
```{r}
writeLines(readLines(root("demos_rstan","grp_prior_mean_var.stan")))
```
Fit the model
```{r, results='hide'}
...
```{r, results='hide'}
fit_grp <- stan(file = 'grp_prior_mean_var.stan', data = d_grp, seed = SEED, control=list(adapt_delta=0.95));
```
